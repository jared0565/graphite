...
If that final transaction fails after the single provider call, the service attempts to stage the validated receipt and marks the attempt `persistence_failed`. Recovery is guaranteed only when this fallback staging succeeds. `recoverable_attempt_ids()` exposes only attempts with a staged receipt, and `reconcile_execution(attempt_id)` transactionally performs the missing finalization without a provider call or approval reuse. If storage is unavailable for both normal finalization and fallback staging, the sanitized `execution_persistence_failed` result cannot guarantee recoverability.

The schema-v2 migration deliberately maps legacy `pending` and `persistence_failed` attempts that lack token and request-hash bindings to `legacy_unrecoverable` with `legacy_attempt_bindings_missing`; they are quarantined and never replayed. Legacy `completed` rows missing the new bindings remain intentionally preserved as read-only history. They are not reclassified as recoverable and are not rewritten to manufacture stronger evidence than the earlier schema recorded.
