...
Manifest and lockfile identities and hashes are captured before mutation and rechecked. Graphite exposes only fixed outcomes, reason codes, manager names, and relative changed control-file paths—not credentials, raw process output, registry responses, absolute host paths, or repository source. The lock excludes concurrent activation only within the current process; other editors and package managers remain possible failure domains.
