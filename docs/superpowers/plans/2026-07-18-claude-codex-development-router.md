...
- [x] Make one no-edit profile-verification call for the selected Claude profile. No retry or fallback. Record only sanitized receipt metadata and human verdict. The original identity was accepted, but a later CLI upgrade invalidated it. One approved free-text refresh failed the exact-response contract. After an offline structured-output fix, one newly approved structured refresh exited without a parseable terminal result; its model and usage remain unknown and no current Claude snapshot exists.
