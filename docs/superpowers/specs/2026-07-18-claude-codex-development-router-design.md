...
The CLIs do not expose a sufficiently stable, authoritative model-inventory command. Graphite therefore uses a maintained allowlist of tested tuples and never invents availability through heuristic discovery. Preflight verifies the resolved executable, executable identity, CLI version, authentication health, and requested profile. Where structured provider output reports the effective model, it must match the approval. If exact effective identity cannot be verified for a profile, that profile cannot receive execution authority.
